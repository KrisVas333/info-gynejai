# P3 · Mentoriaus scenarijus — „Mistė norėjo padėti"

**PAMOKA 3 · LYGIS 0 ▸ ŽINOVAS · KLAVIATŪRA** *(srities etiketė 🟡 pasiūlyta, nepatvirtinta)*
**45 min · kompiuterių klasė · V3.1 · 2026-09-23** *(istorijos blokas perrašytas pagal Kris'o srautą)*
**Mentorius be patirties turi pravesti vien iš šito failo.**

> **Žymėjimas:** **„taip"** = sakoma balsu pažodžiui · ❓ = vedantis klausimas ·
> ▲ = vaiko veiksmas · 🔴 = **čia pamokos miršta, nepraleisti** · 💬 = mentoriaus
> sakinys po atsakymu (§ 8.0 C3) · *(kursyvas)* = mentoriui, negarsiai

> ⚠️ **V3 — istorija ir klaida pakeista nuo V2** *(Kris, 2026-09-08)*. Mistė VĖL turi
> klaidą (V2 ją buvo padaręs be klaidos, „nestandartinė" pamoka — tai atmesta). Nauja
> klaida: **ji nori padėti, bet pati nežino pakankamai.** Atomazga — jie randa
> **Mokytoją Lektą**. Pilnas pasikeitimų sąrašas — `README.md` § Kas pasikeitė nuo V2.
> Nematytas klasėje, nematytas projektoriuje, be 🇱🇹 patikros.

---

## 🔴 PRIEŠ PAMOKĄ

- [ ] 🔴 **KĖDĖS SUSTATYTOS PUSLANKIU PRIE LENTOS** — tiek, kiek vaikų. Abi klasės tai leidžia.
  Vaikai pamokos pradžioje sėdi **ten, ne prie kompiuterių**
- [ ] **Monitoriai NUSUKTI arba UŽDENGTI, dėžutes įjungi TU** — bet to NEUŽTENKA: vaikas prie
  stalo su nusuktu monitoriumi vis tiek yra prie kompiuterio
- [ ] **`klavisu-sokis.html` ant kiekvieno darbastalio** *(kopija iš P2 — ⚠️ dar nenukopijuota į šį aplanką, žr. README)*
- [ ] Žaidimą **atidarei ir praėjai 6–8 lygius pats.** Žinai, kaip atrodo raidžių ir skaičių lygiai
- [ ] **Mokytojos Lektos skaidrė/paveikslėlis paruoštas** — pasirodys Bloko 3 pabaigoje
- [ ] Klasėje yra **laisvos vietos priešais** dviem savanoriams Bloko 3 demonstracijai
- [ ] Projektorius perjungiamas nuo nešiojamo prie klasės kompiuterio ir atgal — **patikrinta**
- [ ] Mokyklos IT žmogus pasiekiamas

> 🔴 **PAMOKA PRASIDEDA ATOKIAU NUO KOMPIUTERIŲ — STANDARTAS, ne šios pamokos išimtis.**
> Ne „monitoriai nusukti kažkur", o **vaikai fiziškai prie lentos**: kėdės sutraukiamos puslankiu,
> ten vyksta kartojimas, istorija ir teorija. **Prie kompiuterių einam tik PO testo ir teorijos** —
> šioje pamokoje Bloke 4, ~27 min (§ 4.6.3 modelis, kaip P1 ir P2). Iki tol prie jų neprieinam.

> ⚠️ **SALIUTAS — visada tas pats gestas kaip P1 (canon.md § Saliutas).** Gilus įkvėpimas →
> abu delnai ant burnos (kairė pirma, alkūnė prigludusi; dešinė ant jos, alkūnė pakelta virš
> pečių). **Jokio „ranka prie kaktos" ir jokio „MES – GYNĖJAI" šūksnio.**

### 🆕 Papildomas pasiruošimas — ŽINOVAI IR SLAPTI LYGIAI

- [ ] **Niekas nerenkamas iš anksto** *(Kris, 2026-09-23)*. Žinovu tampa tas, kuris **pirmas
  pereina visus lygius** — ir tada eina padėti kitiems. Tavo darbas — pastebėti, kas pabaigė,
  ir pasiųsti jį prie to, kas užstrigo.
- [ ] **Žinai slaptų lygių kodą:** žaidimo pradžios ekrane surinkti **`w w w`**. Rodai jį **tik
  tada, kai VISI pabaigė** — tai pamokos prizas, ne užuomina viduryje.
- [ ] Prieš pamoką pats nueik iki slapto kambario, kad žinotum, ką vaikai pamatys.

---

## 0–6 min · BLOKAS 0 · ATIDARYMAS + JUDESIO KARTOJIMAS

### 0:00 · Pasitikimas

Prie durų, **ne prie stalo**. Kiekvienam vaikui atskirai, šiltai:
> **„Labas! Eik sėskis prie lentos — ne prie kompiuterio. O rankytes dar valandėlę
> pailsinam, greitai jas paleisim."**

▶ **PALEISK TAIMERĮ**, kai pirmas vaikas peržengia slenkstį.

*(Tai NĖRA saliutas — saliutas žemiau, taisyklė 4.)*

### 0:40 · Keturios taisyklės

*(Ekrane tik emoji — § 4.7. Tekstą sakai tu.)*

> **„Primenu keturias. ✋ — keliam ranką. 💬 — kalbam po vieną. 💛 — padedam vienas kitam.
> 🤫 — tylim, kai rodau."**

▲ Klasė parodo kiekvieną ranka. Ketvirtai — **tikras SALIUTAS**: gilus įkvėpimas, abu
delnai ant burnos (kaip visada, canon.md § Saliutas). *(~30 sek. Tai ritualas, ne pamoka.)*

### 1:20 · Kaip atsakinėjam

> **„Atsakot ne balsu ir ne ranka. Atsakot kūnu.
> A — pritupiam. B — apsisukam ratu. C — rankos į viršų.
> Aš skaičiuoju TRYS – DU – VIENAS, ir visi kartu."**

### 2:00 · 1/5 — Kodėl skubėti kartais būna lėčiau?

**A** nes pavargsti → *pritūpk* · **B** nes kompiuteris sulėtėja → *apsisuk* · **C** nes taip dažnai priveliame klaidų → *rankos į viršų* — ✅ **C**
> 💬 **„Nes taip dažnai priveliame klaidų. O klaidas paskui reikia taisyti."**

### 2:40 · 2/5 — Ką daro ENTER klavišas?

**A** trina → *pritūpk* · **B** įeina → *apsisuk* · **C** vaikšto → *rankos į viršų* — ✅ **B**
> 💬 **„ENTER įeina. Juo praeitą kartą įėjom teniso naujienas ir vėl grįžom."**

### 3:20 · 3/5 — Klavišų Šokyje, kai raidėje buvo klaida VIDURYJE, ką darėt pirmiausia?

**A** iškart tryniau raides → *pritūpk* · **B** rodyklėmis nuvedėm žymeklį → *apsisuk* ·
**C** pradėjom žodį iš naujo → *rankos į viršų* — ✅ **B**
> 💬 **„Žymeklis pirma. Trynimas antra. Šiandien tai vėl prireiks."**

### 4:00 · 4/5 — Kokia buvo Klaido motyvacija? *(kartojame iš P2)*

**A** nori miegoti → *pritūpk* · **B** nori visko iškart → *apsisuk* ·
**C** nori žaisti → *rankos į viršų* — ✅ **B**
> 💬 **„Nori visko iškart. Todėl jam sunku laukti."**

### 4:40 · 🔴 5/5 — Kokia buvo jo klaida? *(kartojame iš P2)*

**A** užmigo → *pritūpk* · **B** spaudinėjo viską iš eilės ir viskas dingo → *apsisuk* ·
**C** išjungė kompiuterį → *rankos į viršų* — ✅ **B**

🔴 **SUSTOK ČIA — TILTAS Į ISTORIJĄ.** Klaidas vėl sėdi prie klaviatūros šiandien, ir vėl skuba.

### 🆕 5:00 · 🖼️ KODĖL JIS SKUBĖJO *(komikso skaidrė)*

*(Kadre: Klaidas liūdnas prie kompiuterio, virš galvos minties burbulas su sesės Dingailės
veidu. Antrame plane Mistė jį pastebi ir nustemba.)*

> **„O spaudinėjo todėl, kad norėjo kuo greičiau įeiti į džiungles ir rasti savo sesę
> Dingailę. Bet niekaip negalėjo įeiti."**

▶ Rodyk į minties burbulą. Palauk, kol kas nors pastebės mergaitę antrame plane —
*(jei paklaus „o kas ten?" — „Tuoj sužinosit.")*

### 🆕 5:15 · 🖼️ MISTĖ PRISISTATO *(komikso skaidrė)*

*(Kadre: Mistė žvaliai tiesia ranką, virš jos tuščias kalbos burbulas; Klaidas nustebęs —
jis jos nepažįsta.)*

> **„Sveikas! Aš tau padėsiu!"** *(sakyk žvaliai, Mistės balsu)*

> **„Klaidas jos net nepažįsta. O ji jau tiesia ranką."**

⚠️ Vardo dar NESAKOM — ji prisistato Bloke 1.

---

## ~5:30 · ❓ ŠIANDIENOS KLAUSIMAS

> **„Šiandienos klausimas:**
> **Ar visada užtenka NORĖTI padėti?"**

> **„Neatsakinėkit. Įsidėmėkit. Pabaigoje grįšim."**

---

## 6–9 min · BLOKAS 1 · VEIKĖJAS

### 6:10 · Nauja draugė

*(Pradedi nuo ką tik duoto atsakymo — nesustok, nedaryk naujos įžangos.)*

> **„Teisingai. Klaidas vėl bandys. Bet šiandien džiunglėse pasirodo kažkas naujo — mergaitė."**

🖼️ *(Rodyk Mistę pirmą kartą — visa ekrano skaidrė, be teksto.)*

> **„Jos vardas — Mistė. Ji drąsi ir gera, ir labai nori turėti tikrų draugų."**

### 6:50 · Kodėl Mistė šiandien čia

❓ **„Kaip manot, ką daro žmogus, kuris labai nori turėti tikrų draugų?"** ▲ Priimk
3–4 atsakymus. **Netaisyk.**

> **„Mistė visada ieško, kam padėti. Kai kas nors užstringa — ji pirma pastebi
> ir pirma pasisiūlo."**

### 7:40 · 🔴 Motyvacija — pirmas kartas

*(Čia atskleidžiama tai, kas visus metus paaiškins kiekvieną jos klaidą. Sakyk LĖTAI.)*

> **„Ir dabar aš jums pasakysiu apie Mistę vieną dalyką, kurį reikia atsiminti visiems metams."**

> **„Mistė nori turėti tikrų draugų. Ji galvoja, kad padėjus žmonėms, jie norės tapti
> mūsų draugais. Todėl ji padeda — visada, visiems, iškart."**

*(Skaidrėje: 🤝 · MISTĖ NORI TURĖTI TIKRŲ DRAUGŲ · paantraštė „ji galvoja, kad padėjus
žmonėms, jie norės tapti mūsų draugais" — P2 „Klaidas nori visko iškart" maketas,
paantraštė dvigubai didesnė.)*

❓ **„Ar tai gerai?"** *(Kelios rankos — leisk pasiginčyti, netaisyk dar.)*

> **„Skamba gerai, tiesa? Pažiūrėkim, kas nutiko, kai ji šiandien pasisiūlė padėti."**

---

## 9–14 min · BLOKAS 2 · ISTORIJA

*(🆕 Ši istorija TĘSIA praeitos pamokos istoriją — Klaidas vis dar prie tos pačios
klaviatūros. Tonas šiltas ir juokingas — klaida čia grąžina komišką „vėl nepavyko",
ne baimę.)*

### 9:10 · 🎞️ KAS IŠ TO IŠĖJO *(animuotas kadras — tas pats, kaip išvadoje)*

*(Kadras sukasi ratu: Mistė spaudžia klavišus, klavišai lakioja, ekrane raizgalynė,
abu susipainioję.)*

> **„Ji galvojo, kad padeda, bet iš tiesų dar labiau sujaukė reikalus."**

⚠️ Vienas sakinys ir toliau — visos istorijos čia dar nepasakojam.

### 11:30 · 🖼️ 🔴 VIS DAR PRIE VARTŲ *(komikso skaidrė)*

*(Kadre: Klaidas piktas ant Mistės, o ji drovisi ir raudonuoja. Ekrane — raizgalynė.)*

> **„Bet Mistė iš tikrųjų NEŽINOJO klaviatūros taip gerai, kaip pati manė.
> Ir viskas dar labiau susimaišė."**

🎭 *(Sakyk su šypsena, ne su nusivylimu — komiška pauzė:)*

> **„Ir taip... trys pamokos praėjo, o Klaidas su Miste vis dar stovi prie tų pačių
> džiunglių vartų!"**

*(Klasė turi nusijuokti čia — tai savikritiškas juokas apie tempą, ne apie vaikus.)*

### 12:30 · ❓ Kodėl Mistės pagalba nepadėjo?

❓ **„Kodėl, jūsų manymu, Mistės pagalba viską pablogino, o ne pagerino?"**

▲ Leisk pasiūlyti. *(Kryptis: ji norėjo padėti, bet pati nežinojo.)*

> 💬 **„Mistė NORĖJO padėti. Bet norėti padėti ir MOKĖTI padėti — ne tas pats."**

🔴 **IR ČIA — SAVAIS ŽODŽIAIS, ramiai:**

> **„Kai nežinai, ką darai, tu niekam nepadedi. O kartais net draugystę sugadini —
> žiūrėkit, kaip dabar jiedu stovi."**

> **„Tai ką daryti? Reikia paklausti to, kuris ŽINO. Toks žmogus vadinasi ŽINOVAS."**

> **„Mokytojai žino daug. Bet šiandien mokytojas čia tik vienas — o jūsų daug.
> Tai kur rasti daugiau žinovų? Tuoj pamatysit."**

🔴 *(Taisyklės dar NESAKOM. Ji gimsta po termino kortelės.)*

---

## 14–26 min · BLOKAS 3 · ATRADIMAS 🔴

> 🔴 **EKRANAI VIS DAR NEJUNGIAMI.** Visas šitas blokas vyksta klasės priekyje ir
> projektoriuje — be vaikų įrenginių, be ekrano.
>
> **§ 7 keturi žingsniai:** ĮVARDIJIMAS → PROGNOZĖ → FIZINĖ DEMONSTRACIJA (be įrangos,
> klasė kūnu) → TAISYKLĖ gimsta pabaigoje, sakoma vaikų, ne mentoriaus.

### 14:00 · Kas skyrėsi — norėti ar žinoti

❓ **„Mistė labai norėjo padėti. Ko jai trūko, kad pagalba iš tikrųjų pavyktų?"**

▲ Leisk pasiūlyti 2–3 variantus. *(Kryptis: žinojimo, ne noro.)*

### 15:10 · 🔴 TERMINO KORTELĖ · ŽINOVAS

> **„ŽINOVAS — tas, kuris TIKRAI žino. Kai nei vienas draugas nežino, pas žinovą galima kreiptis."**

*(Užrakintas maketas — § 8.0 C2. Toks pat maketas kaip P2's KLAVIŠAS. Perskaityk balsu ir sustok.)*

> **„Dabar patys pamatysim, kaip skiriasi spėjimas nuo žinojimo."**

### 20:00 · ❓ Tai ką daryti, kai nei vienas nežino? *(viena skaidrė su Lekta)*

❓ **„Tai ką daro Klaidas ir Mistė, kai nei vienas jų nežino?"**

▲ Leisk pasiūlyti. *(Kryptis: paklausti to, kuris žino.)*

> 💬 **„Reikia rasti ŽINOVĄ."**

▶ **Toje pačioje skaidrėje — Mokytoja Lekta.**

> **„Ir tada džiunglėse pasirodė Mokytoja Lekta. Ji ŽINO klaviatūrą — ne spėja, ŽINO.
> Ji parodė Klaidui ir Mistei tikrą kelią."**

> **„Nuo šiandien atsiminkit: jeigu neaišku — klausk Lektos."**

🔴 *(Pirmas kartas, kai šita taisyklė pastatoma pamokoje, ne tik parašyta kanone.
Sakyk aiškiai ir sustok — tegul įsimena kaip atskirą sakinį.)*

### 24:00 · 🔴 TAISYKLĖ GIMSTA *(sako vaikai, ne tu)*

> **„Kai nei vienas nežino — randi tą, kuris žino."**

🆕 **PEREINAM Į INTERAKTYVĄ — klausimu, ne paaiškinimu:**

> **„Tai gal kas nors, kuris ŽINO, man primins — kaip veikia tarpas? O kaip veikia
> BACKSPACE?"**

▲ Vaikai atsako balsu. *(Priimk bet kokį teisingą — svarbu, kad atsakytų jie.)*

> 💬 **„Va. Žinovas jau yra šitoj klasėj."**

*(Ir iškart — prie mentoriaus klaviatūros.)*

### 🆕 24:40 · RAŠOM PRIE MANO KLAVIATŪROS *(skaidrė „Rašom!")*

*(P2 rašymo demo — vaikai ją mėgo labiausiai, todėl ji grįžta. Vaikai tebesėdi prie lentos;
prie savo kompiuterių dar neinam. Klaviatūra — tavo, prie projektoriaus.)*

1. Rašai **KATĖ** — lėtai, garsiai vardydamas raides.
2. 🔴 Rašai antrą **T** viduryje: **KATTĖ**.
   > **„Oi. Apsirikau — ir ne gale, o viduryje."**
3. ❓ **„Ar BACKSPACE ištrins tą raidę iš čia?"** ▲ Leisk pabandyti — ištrina **Ė**, ne T.
4. ⬅️ Nuvedi žymeklį prie klaidos, skaičiuodamas garsiai. ⌫ — dingsta tik T.
5. 💬 **„Žymeklis pirma. Trynimas antra."**

### 🆕 25:20 · 🔴 PENKI TAISYTOJAI *(skaidrė „Penki žodžiai — penkios klaidos")*

🔴 **Čia atsiperka P2 sunkiausias lygis** — klaida žodžio viduryje. Penki vaikai **po vieną**
ateina prie tavo klaviatūros; kiti sėdi prie lentos ir žiūri.

Ekrane penki laukeliai su mūsų pamokų žodžiais — **dvi skirtingos užduotys**:

| # | Laukelyje | Ką reikia padaryti |
|---|---|---|
| 1 | **EEKRANAS** | perteklinė raidė pradžioje → ⌫ |
| 2 | **BAITTAS** | perteklinė raidė viduryje → ⌫ |
| 3 | **ŽINNOVAS** | perteklinė raidė viduryje → ⌫ |
| 4 | **KLAVIŠŲŠOKIS** | 🆕 du žodžiai surašyti kartu → **TARPAS** reikiamoje vietoje |
| 5 | **MOKYTOJALEKTA** | 🆕 du žodžiai surašyti kartu → **TARPAS** reikiamoje vietoje |

Skaidrės kairėje **ratu sukasi animacija**: rankos kontūras smiliumi paspaudžia ⬅, paskui ⌫,
paskui **tarpo klavišą** — trys judesiai, kurių reikia šiai skaidrei.
Tai vaiko atmintinė — jei užmiršta, rodyk į ją, o ne sakyk.

Kiekvienam vaikui ta pati tvarka:
- ❓ **„Kas čia negerai?"** ▲ Vaikas pirma **parodo pirštu ekrane** — tik tada liečia klaviatūrą.
  *(4 ir 5 klausimas kitoks: „Kiek čia žodžių? Kur jie turi išsiskirti?")*
- ▲ ⬅️ ➡️ nuveda žymeklį, ⌫ ištrina. **Ištaisytas žodis pats užsidega žaliai** — tau sakyti
  nieko nereikia *(§ 8.1 — atsakymas rodomas spalva, ne tekstu)*.
- 💬 **„Žymeklis pirma, trynimas antra."**
- ⭾ **TAB** (arba spustelėjimas pele) perduoda eilę kitam laukeliui. Aktyvus laukelis šviečia.
- ⚠️ **Šitoje skaidrėje TARPAS rašo tarpą** (jis yra užduoties dalis), todėl skaidrę verčia
  **ENTER** arba ➡️ mygtukas apačioje kairėje — ne tarpas.

⚠️ **Sunkiausi — 4 ir 5**: ten nieko trinti nereikia, reikia **pridėti** tarpą, o vaikai pirmiausia
griebiasi ⌫. Palik juos drąsesniems ir neskubėk pasakyti — tegul klasė pasiūlo. Jei kuris užstringa ilgiau nei 30 s — **padeda visa klasė balsu, ne tu.**
Tai ta pati šios pamokos taisyklė: kai pats nežinai, randi tą, kuris žino.

📄 **MOKYTOJAI:** pasižymėk, kiek iš penkių rado klaidą **be pagalbos** — tai tiesioginis
P2 sunkiausio lygio matavimas.

---

## 26–38 min · BLOKAS 4 · TRENIRUOTĖ

> ⚠️ **Kampų žaidimo šitoje pamokoje nėra** *(design-standards § 4, nebeprivalomas
> kiekvienoje pamokoje)*. Bloko 3 spėjimo/žinojimo demonstracija jau buvo šios pamokos
> nepertraukiamas fizinis elementas.
>
> 🆕 **Blokas 4 yra Bloko 3 atradimo tiesioginė praktika: GREITIEJI PADĖJĖJAI.**
> Šaltinis: Kris x Gabrielius pokalbis 2026-09-04. **Skirtumas nuo Mistės klaidos:**
> Mistė norėjo padėti, bet nežinojo. Greitieji Padėjėjai TIKRAI žino — jie baigė
> šitą pačią žaidimo dalį P2 metu. Jų pagalba veikia, nes jie **yra žinovai, ne
> spėliotojai.** Sąmoningai priimtas antras kintamasis lockstep lange — žr.
> `frontier.md` § Adopted by decision.

### 26:00 · Štai jis, vėl

> **„Štai jis vėl — Klavišų Šokis, jūsų darbastalyje."**

Rodyk į ikoną skaidrėje. **Vaikai vis dar sėdi prie lentos — ekranai tamsūs, nelieti.**

### 27:00 · 🔴 ĮEINAM 1·2·3 — pirmas ekrano prisilietimas

🔴 **Nuo šitos minutės — ir ne anksčiau — vaikai turi ekraną.**

▲ **PIRMA PEREINAM.** Tik dabar kėdės grąžinamos prie stalų:
> **„Dabar tyliai keliamės, kėdę stumiam prie savo kompiuterio ir sėdamės.
> Rankos ant kelių, kol pasakysiu."**

*(Palauk, kol VISI sėdi ir rankos ant kelių. Tik tada — 1·2·3.)*

👁️ **RODAU** pirmas, projektoriuje → ▲ **KARTOJAM:**
> **„TRYS — DU — VIENAS — SPAUDŽIAM."**

📄 **MOKYTOJAI:** skaičiuojam **A** (kiek vaikų reikėjo gelbėti po vieną) ir
**C** (kelintą minutę užsidegė paskutinis ekranas).

### 28:00 · 🔴 ŽINOVAI IR SLAPTI LYGIAI *(atskleidimas VISAI klasei)*

*(Šitą sakai visiems, ne tik greitiesiems — tai šios pamokos pažadas.)*

> **„Šiandien kai kurie iš jūsų irgi bus mokytojai."**

> **„Kas pirmas pereis visus lygius — eina padėti kitiems. Bet NE spausti už juos!
> Tik sakyti, kurį klavišą spausti. Kaip Lekta, ne kaip Mistė."**

> **„Žinovo ženklas — rankos už galvos. Štai taip."** ▲ Vaikai pakartoja judesį.

🗝️ **IR PRIZAS:**

> **„Ir jei VISI pabaigsit — aš jums parodysiu, kaip patekti į slaptus lygius."**

⚠️ **Kodo nesakyk, kol visi nepabaigė.** Slaptas įėjimas: žaidimo pradžios ekrane
surinkti **`w w w`** (trys iš eilės). Antras slaptas kambarys atsiveria už pirmojo.

❓ **„Ar Lekta paspaudė klavišus už Klaidą, ar tik parodė kelią?"** ▲ *(Tik parodė.)*

> 💬 **„Tiksliai. Žinovas parodo kelią. Nepadaro už kitą."**

📄 **MOKYTOJAI:** pasižymėk, kas pirmas baigė ir kam padėjo.

### 29:00 · Kitiems vaikams — kas jų laukia

*(Šitą sakai VISIEMS, po to, kai padėjėjai gavo savo instruktažą.)*

> **„O jeigu kažkas užstrigs — nekelkit rankos iškart. Pirma pažiūrėkit, ar šalia
> sėdi Žinovas su rankomis už galvos. Jis jums pasakys, ką spausti."**

📄 **MOKYTOJAI:** pirmas 5 minutes žaidimo **nesikišk pati, jei tik gali** — leisk
padėjėjų mechanikai pasireikšti. Po 5 minučių įsijunk, jei kas nors vis tiek strigs.

### 30:00 · 🔴 Klavišų Šokis — 6–8 lygiai

> 🔴 **Šiandien einam LYGIUS 6–8** *(raidės · skaičiai · viskas kartu)* — P2 sustojo
> ties 5-uoju. 9–10 lygiai *(Gynėjo egzaminas)* lieka namams arba kitai pamokai.

🎮 **Žaidimas yra ŠITOJE skaidrėje**, kaip P2. Projektoriaus neperjunginėk.

▶ **Prieš 6 lygį** — mentorius spusteli mygtuką „6 LYGIS — TIK SU MOKYTOJU".

| Lygis | Kas jame | Ką pasakai prieš |
|---|---|---|
| **6** | raidės | *„Dabar raidės. Jos rašo — prisimenat?"* |
| **7** | skaičiai | *„O dabar skaičiai — jie irgi rašo, tik kitką."* |
| **8** 🔴 | viskas kartu | žr. žemiau |

**33:00 · 🔴 Paskelbtas sunkėjimas** *(§ 8.0 C4)*

> **„Puiku! O dabar — sunkiausias. Šitame lygyje viskas maišosi — raidės, skaičiai,
> rodyklės, ir gali būti klaida bet kur."**

▲ Vaikai žaidžia, žinovai vaikšto tarp jų su rankomis už galvos.

📄 **MOKYTOJAI:** po 8 lygio skaičiuojam **B** — kiek rankų pakilo prie
❓ **„Kam pavyko dėl draugo pagalbos?"**

⚠️ Jei žaidimas turi savo pertraukos ekraną po 8 lygio (kaip P2 turėjo po 5-o) —
**tai ir yra tavo sustojimo taškas.** Toliau neik.

*(Jei lieka laiko — kartok 8 lygį. **NEEIK į 9-tą.** Gynėjo egzaminas — kitos pamokos.)*

---

## 38–42 min · BLOKAS 5 · IŠVADA

### 38:00 · Šiandienos klausimo atsakymas

*(Rodyk tą pačią skaidrę, kaip 5:30. Ta pati eilutė, žodis žodin — § 8.0 C1.)*

> **„Šiandienos klausimas buvo šitas:"**
> **„Ar visada užtenka NORĖTI padėti?"**

❓ **„Ir kaip?"** ▲ Klasė atsako.

> 💬 **„Ne. Norėti padėti yra gražu, bet dar reikia ŽINOTI, kaip. O kai nežinai —
> randi tą, kuris žino."**

### 39:00 · 🔴 IŠVADA — choru, du kartus

*(Žodis žodin. Iš šitos eilutės generuojamas kitos savaitės kartojimo klausimas.)*

> **„Kartojam kartu: Didžioji Mistės klaida buvo —
> ji labai norėjo padėti, bet pati dar nežinojo pakankamai, ir jie abu dar labiau susipainiojo."**

▲ **Visa klasė, du kartus.**

### 40:10 · 🔴 Klysti yra normalu — ir dabar mes žengiam giliau

*(Šitą sako mentorius, ne choras. Ir sako šiltai — tai pergalė, ne pamoka.)*

> **„Mistė suklydo. Bet ji surado Lektą — žinovę, kuri parodė tikrą kelią."**

> **„Mistė klysta, kad tau nereikėtų."**

> **„Ir dabar — pagaliau — po trijų pamokų, mes žengiam į džiungles giliau."**

🔴 *(Ši eilutė yra sąmoningas komiškos „vis dar prie vartų" linijos uždarymas —
sakyk su šypsena, klasei turi būti smagu, kad „pagaliau".)*

### 41:00 · Atsisveikinimas

🔴 **Saliutas — jei reikia klasei nutildyti, tas pats gestas kaip visada.**

> **„Gynėjai, iki kito karto."**

---

## 42–45 min · BLOKAS 6 · NAMŲ DARBAS

### 42:00 · Rask žinovą

> **„Namų darbas: šią savaitę, kai kažko nežinosi — nespėliok su draugu. Paklausk
> žinovo — mamos, tėčio, mokytojos, kažko, kuris tikrai žino."**

> **„Ir papasakok jiems apie Mistę — kad ji norėjo padėti, bet pati nežinojo. Paklausk:
> ar jiems kada nutiko tas pats?"**

### 43:20 · Cliffhanger

> **„Kitą kartą džiunglėse sutiksim dar vieną. Jis pažins jus geriau, nei jūs patys
> save pažįstat — bet dėl to irgi suklys."**

*(Nesakyk vardo. Bridge į P4 = Neuronas. 📌 P4 planas — žr. `canon.md`: pamoka
turėtų baigtis konkrečia taisykle „į džiungles vieni neinam, jei randi ką nors
keisto — parodyk suaugusiam", uždarant P3→P4 loką.)*

---

## 📄 Klasės mokytojos lapas *(spausdinamas, 1 psl.)*

**Penki darbai per pamoką.**

1. **A** — kiek vaikų prireikė gelbėti po vieną per **ĮEINAM** (27:00–29:00): `____`
2. **B** — kiek rankų pakilo prie *„Kam pavyko dėl draugo pagalbos?"* po **8 lygio** (~37:30): `____`
3. **C** — kelintą minutę užsidegė **paskutinis** ekranas: `____`
4. **D** — kiek kartų mokytoja PATI turėjo įsikišti Bloke 4 (ne žinovas-padėjėjas): `____`
5. 🆕 **E** — kiek iš penkių taisytojų (25:20) rado klaidą **be pagalbos**: `____`

*Šitie skaičiai — lockstep + peer-support matavimas kartu. Nuo P3 lyginimas
jau nebėra švarus vieno kintamojo eksperimentas (žr. `frontier.md`). Jei lapas
nesurenkamas, D niekada neišmatuojamas.*

---
← [[pamoka 9 what]] · [[../../wiki/index|Turinio žemėlapis]]
